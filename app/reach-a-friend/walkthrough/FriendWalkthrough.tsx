...
export default function FriendWalkthrough() {
  ...
  function choosePath(path: Pathway) { setSelected(path); setStep(0); setAnswers([]); setPicked(null); setCopied(false); requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: reducedMotion ? "auto" : "smooth" })); }
  function next() { if (picked === null) return; window.speechSynthesis?.cancel(); setSpeaking(null); setAnswers([...answers, picked]); setStep(step + 1); setPicked(null); requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: reducedMotion ? "auto" : "smooth" })); }
  async function copyTemplate() { if (!selected) return; await navigator.clipboard.writeText(selected.template); setCopied(true); }

  return <main className={`friend-sim ${reducedMotion ? "reduce-motion" : ""}`}>
    <header className="friend-sim-nav"><Link href="/">← REACH Action Hub</Link><div className="friend-nav-actions"><button type="button" onClick={() => setReducedMotion(value => !value)} aria-pressed={reducedMotion}>{reducedMotion ? "Turn motion on" : "Reduce motion"}</button><Link href="/reach-a-friend">Friend support guide</Link></div></header>
    <a className="friend-safety" href="https://988lifeline.org/" target="_blank" rel="noreferrer"><span>Need help now?</span><b>988</b></a>
